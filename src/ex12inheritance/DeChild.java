...
public class DeChild extends DeParent {
	...
	public void study() {
		/*
		 ...
		 자식에서는 getter를 통해 값을 반환받아야 한다.
		 */
		System.out.println(getName() + "이(가) 공부한다.");
		//아래와 같이 직접 접근하염 에러 발생함
//		System.out.println(name() + "이(가) 공부한다.");
	...
	/*
	 ...
	 1. 부모클래스에서 선언될때와 비교해서 접근범위가 동일하거나 
	 	그보다 넓으면 가능하다.
	 2. 부모에서 default로 선언하면 자식에서는 default 혹은 
	 	그보다 넓은 public으로 선언하면 오버라이딩이 성립한다.
	 */
	@Override
	public String sleep() {
	...
	public void printParent() {
		/*
		 ...
		 2. super.멤버: 부모의 멤버변수 혹은 멤버메서들르 호출한다.
		 */
		super.printParent();
