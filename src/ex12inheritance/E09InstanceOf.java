...
-부모타입의 참조변수로 자식인스턴스를 참조할 수 있으므로 
 매개변수를 부모타입으로 정의하면 자식인스턴스를 인수로 받을 수 있다.
-매개변수로 전달되는 인수는 부모타입으로 자동형변환(업캐스팅)되므로 
 정상적인 사용을 위해 강제형변환(다운캐스팅)해야 하는데 이때 확인을 위해 사용된다.
 */
...
public class E09InstanceOf {
	/*
	 -메서드 정의 시 부모타입의 매개변수를 설정하면 
	  자식타입의 인스턴스를 모두 받을 수 있다.
	 -Box타입으로 인수를 받게 되면 자동형변환(업캐스팅)되므로 
	  자식클래스의 멤버메서드는 호출할 수 없게 된다.
	 -이때 자식의 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)이 필요하다.
	 -instanceof 연산자를 통해 자식 타입으로 형변환이 가능한지 판단 후
	  true를 반환하면 다운캐스팅을 진행한 후 메서드를 호출할 수 있다.
	 */
	static void wrapBox(Box b) {
		/*
		 세가지 인스턴스 중 어떤것이 전달되더라도 Box타입으로 자동형변환되므로 
