...
class MyChild extends MyParent {
	...
	/*
	 메서드명은 동일하지만 매개변수가 다르므로 오버로딩한 메서드임. 
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		myChild.childMethod();
		//매개변수 있음->오버로딩한 메서드(확장)
		myChild.parentMethod(1000);
		//매개변수 없음-> 오버라이딩해서 재정의한 메서드(자식쪽의 메서드가 호출됨)
		myChild.parentMethod();
		
		System.out.println();
		System.out.println("[부모타입에 자식타입의 메모리 주소복사]");
		/*
		 이질화: 부모타입의 참조변수로 자식인스턴스를 참조한느 것을 말한다.
		 이 경우 자식 클래스에서 새롭게 정의한 멤버는 접근할수없다.
		 ...
		 */
//		(MyChild)myParent.childMember =1;//에러발생
		...
		/*
		 ...
		 따라서 모든 인스턴스는 Object의 참조변수로 참조 가능하다.
