...
/*
 ...
 -main() 메서드가 실행되기 전에 초기화되므로 
  main() 메서드 실행시 별도의 인스턴스 생성 없이 변수를 생성할 수 있다.
...
class MyStatic {
	...
	/*
	 정적 멤버메서드: 정적 메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
	 			  정적 멤버만 접근할 수 있다.
	 	 (* main메서드에서는 static으로 선언된 메서드만 호출할 수 있었음을 기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 Java에서는 권장하지 않는다.
		 참조변수를 통해 접근할거면 굳이 정적변수를 사용할 이유가 없기 때문.
		 */
		myStatic.staticVar = 300;
		System.out.println("myStatic.staticVar="+myStatic.staticVar);
		
		/*
		 2개의 인스턴스를 생성한다.(Heap영역)
		 */
...
//		ms2.staticVar =900;
		/*
		 따라서 위의 문장은 권장되지 않는다. 
		 코드레벨에 보면 서로 다른 변수에 값을 할당하는 것으로 보이기 때문이다.
		 아래 문장을 권장한다. 
