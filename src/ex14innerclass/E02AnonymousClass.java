...
/*
 ...
 -클래스를 정의하는 것이 마치 메서드를 정의하는 것봐 비슷하다.
...
class Singer {
	...
	/*
	 toString(): Object 클래스에 정의된 메서드로 
	 객체가 가진 값을 문자열로 반환해준다. 
	 Java에서 정의하는 모든 클래스는 Object클래스를 
...
//			boolean mic = true;
		...
		/*
		 부모의 참조변수로 자식을 참조할 수 있지만, 접금범위가 부모까지로 제한된다.
		 따라서 아래와 같이 자식에서 확장한 멤버변수는 접근할 수 없어 에러 발생한다.
		 접ㄱ느하고 싶다면 다운캐스팅(강제형변환)을 해야하지만 
...
//		System.out.println(s3.mic); //에러발생
		...
		/*
		 ...
		 따라서 인터페이스에 정의된 추상메서드를 오버라이딩하여 
		 익명클래스를 만드는 경우가 매우 많다.
		 */
