...
public class E03CallByValue {
	...
	public static void main(String[] args) {
		...
		/*출력2: 매개변수로 전달된 값은 복사되어 다른 지역의 메서드 내에서만 변경되므로 
				main에서는 초기값이 그대로 유지된다.*/
		System.out.println("[main메소드안-호출후]"+"first="+first+", second="+second);
	}
	
	/*
	 매개변수로 전달된 값은 복사되어 해당지역의 메서드에서 사용할 수 있게 된다.
	 즉 매개변수의 전달은 메모리의 복사에 의해 이뤄진다.
	 */
	public static void callByValue(int fNum, int sNum) {
		/*
		 Swap(교환): 두개의 변수가 할당받은 값을 서로 교환하는 알고리즘을 말한다.
		 			교환 시에는 임시변수(temp)가 하나 추가로 필요하다.
		 */
		int temp;
		temp = fNum;
		fNum = sNum;
