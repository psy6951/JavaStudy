...
/*
 ...
 -반환값이 없다. 따라서 선언시 void를 사용하면 안되다.
 -생성자 내부에서 또다른 생성자를 호출하 수 있다.
 -오버로딩이 가능하다.
 */
public class PersonConstructor {
	...
	/*
	 개발자가 생성자를 직접 정의하지 않으면 아래와 같이 매개변수와 싱행부가 없는
	 디폴트 생성자가 컴파일러에 의해 자동으로 삽입된다.
	 지금까지 인스턴스 생성시 new 클래스명()과 같이 사용했던 문장이
	...
	void initialize(String name, int age, String addr) {
		/*
		 ...
		 생성자는 인스턴스 생성시 딱 한번만 호출되고, 개발자가 임의로 호출할수 없다.
		 하지만 멤버베서드는 개발자가 원할때 언제든 호출이 가능하므로 생성자와는 특성이 다르다.
		 따라서 멤버베서드에서는 생성자를 호출할 수 없다.
		 */
