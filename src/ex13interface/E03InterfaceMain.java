...
/*
 인터페이스(interface)
 -클래스가 객체의 설계도라면, 인터페이스는 클래스 상속 간의 설계도라 할수있다.
 -자바는 단일상속을 원칙으로 하지만, 인터페이스를 통해 다중상속을 구현할수있다.
 -추상클래스와 동일하게 상속을 목적으로 생성하므로 
  구현받은 하위클래스에서는 반드시 추상메서드를 오버라이딩 해야한다.
...
class MyClass extends SimpleClass implements MyInterface1, MyInterface2 {
	/*
	 ...
	 오버라이딩은 항상 자식쪽의 메서드가 호출되므로 아무 문제도 발생하지 않는다.
	 */
	@Override
	...
	/*
	 SimpleClass 클래스의 멤버메서드는 필요한 경우에만 오버라이딩하면 된다.
	 필수사항이 아니므로 에러가 발생하지는 않는다.
	 */
}
public class E03InterfaceMain {
	...
	public static void main(String[] args) {
		...
		/*
		 MyClass의 부모는 모두 3개이므로 아래는 모두 참조가 가능한 코드이다.
		 */
