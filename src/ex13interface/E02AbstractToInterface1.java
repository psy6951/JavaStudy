...
/*
 ...
 해당 클래스는 추상메서드를 포함하고 있으므로 
 하위에서 상속받으면 반드시 추상메서드를 오버라이딩해야한다.
 */
...
/*
 DTO(Data Transfer Object): 데이터를 저장하고 전송하기 위한 용도의 객체를 말한다. 
 							...
 							해당프로그램에서는 이름과 주민번호를 저장하는 용도로 사용된다.
 */
class PersonalInfoDTO {
	...
	/*
	 getter/setter메서드 정의
	 : private으로 정의된 멤버변수를 설정하거나 값을 반환받기 위해 생성함
	 */
...
//추상 클래스를 상속하여 정보 저장 및 검색 기능을 구현한 클래스
class PersonalNumberStorageExt extends PersonalNumberStorageAbs{
	
	/*
	 멤버 변수: 2명 이상의 정보를 저장해야 하므로 DTO 타입의 배열을 선언.
	...
	public String searchPersonalInfo(String juminNum) {
		//배열에 저장된 인스턴스의 개수만큼 반복
			for(int i=0 ; i<numOfPerInfo ; i++) {
				//매개변수로 전달된 주민번호를 통해 배역 검색
				if(juminNum.compareTo(personalArr[i].getJuminNum())==0)
				{
					//일치하는 정보가 있는 겨우 이름을 출력한다.
