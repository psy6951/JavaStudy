...
//		Vector<String> list = new Vector<String>();
		...
		/*
		 인스턴스 저장: add()
		 add(인스턴스): 순차적으로 저장하면서 인덱스는 0부터 자동부여
		 add(인덱스,인스턴스): 데이터 저장 시 인덱스 직접 부여. 
					 단, 인덱스를 건너뛰면 에러발생하므로 주의해야 한다.
		...
		list.add("트와이스");
		list.add(3,"워너원");
//		list.add(5,"소방차"); //런타임 에러발생(실행전엔 에러 인지 어려움)
		/*
		...
		/*
		 중복저장: List는 배열의 특성을 가지므로 데이터 중복저장 가능
				 동일한 데이터라도 index로 구분할수 있기 때문.
				 add()는 인스턴스 추가에 성공할 경우 true를 반환한다.
		 */
		System.out.println(list.add("트와이스")? "중복저장됨":"중복저장 안됨");
		System.out.println("중복저장 후 객체수:"+list.size()); //7개
		...
		/*
		 ...
		 방법1: 일반 for문 사용. 
		 		이때는 인덱스를 통해 접근해야 하므로 get()메서드 사용.
		 */
		System.out.println("\n[일반for문 사용]");
		...
		/*
		 방법2: 확장for문 사용.
		 	확장 for문의 경우 인덱스값이 전체 원소를 반복해주므로 
		 	좀더 간단한 표현이 가능하다. 가장 많이 사용되는 방식이다.
		 */
		System.out.println("\n[확장for문 사용]");
		for (String obj : list) {
		...
		/*
		 방법: 반복자(Iterator) 사용
		 1. 컬렉션에 저장된 내용을 Iterator 객체에게 알려주기 위해 인스턴스 생성
		 2. hasNext()로 인출할 인스턴스가 있는지 검사 후, 만약 없다면 false 반환
		...
		/*
		 수정하기(덮어쓰기)
		 : 기존의 내용을 변경한다. 즉 기존 내용 삭제 후 새로운 내용으로 갱신.
		 */
		...
		/*
		 삭제하기
		 방법1: 인덱스로 삭제
			 remove(인덱스): 인덱스를 통해 삭제 후 삭제완료후에는 해당 인스턴스를 반환.
			 			삭제후에는 인덱스가 자동으로 부여됨. 
			 			삭제 성공시 해당 인스턴스의 참조값을 반환하므로 삭제 즉시 확인 가능
		 */
		//2번 인덱스 삭제 =>'트와이스' 삭제 후 반환함
		...
		/*
		 전체삭제: 아래 2개의 메서드를 통해 컬렉션에 저장된 전체 데이터 삭제
		 */
		list.removeAll(list);
		list.clear();
