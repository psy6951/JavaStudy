...
public class Ex08AsList {
	...
	public static void listPrint(String title, List<String> list) {
		...
		for (Object ob: list) {
			/*
			 출력할 인스턴스가 String이므로 
			 별도의 오버라이딩 없이도 내용 출력 가능.
			 만약 우리가 직접 정의한 클래스라면 toString()을 오버라이딩 해야함
			 */
			System.out.print(ob+" ");
		}
		System.out.println();
	...
	public static void main(String[] args) {
		...
		이 경우 초기데이터를 통해 크기를 유추할 수 있으므로 크기는 선언문장에서 생략해야 함 
		 */
		String[] strArr = new String[] {"마린","파이어뱃","메딕", "마린"};
		/*
		 Arrays.asList(): 일반배열을 List<T> 컬렉션으로 변경할 때 사용.
		 				단, 값을 참조만 할 수 있고 입력이나 삭제는 불가능함
		 */
		List<String> list =Arrays.asList(strArr);
		listPrint("출력1",list);
		//현 상태에서는 데이터를 추가할 수 없으므로 런타임에러 발생
//		list.add("탱크"); //런타임 에러
		...
		/*
		Set<E> 컬렉션은 중복을 허용하지 않으므로 
		List에 저장된 중복값을 제거할 수 있다.
		*/
