...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 인스턴스 생성시 Student 형의 타입 매게변수를 통해 생성한다.
...
//		ArrayList<Student> list2 = new ArrayList <Student>();
		...
		list2.add(st3);
		list2.add(st4); //인덱스3
		list2.add(st2); //리스트이므로 중복저장 가능함(인덱스로 구분가능)
		
		System.out.println("[출력1-일반for문]");
		/*
		 일반 for문 사용을 위해 size()메서드로 추가된 인스턴스의 개수를 반환받는다.
		 또한 index로 접근해야 하므로 get()메서드를 사용한다.
		 */
		for(int i=0 ; i<list2.size() ; i++) {
			/*
			 List의 각 인덱스에 저장된 인스턴스의 참조값을 그대로 출력함.
			 toString()메서드를 오버라이딩 했으므로 인스턴스에 저장된 멤버변수값이
		...
		/*
		 ...
		 반환할 인스턴스가 있는지 확인한 후 true이면 next()로 출력함.
		 */
		...
		/*
		 시나리오] 컬렉션에 저장된 인스턴스를 이름으로 검색하여 삭제하는 코드를
