...
public class Ex07MyFriendInfoBook {
	...
	/*
	 ...
	 따라서 선ㄴ택한 메뉴에 따라 핸들러 클래서의 메서드만 호출하는 형태로 구현되어 있다.
	 */
...
//	private int numOfFriends;
	...
	/*
	 기존 인스턴스 배열에 저장했던 것을 컬렉션으로 변경한다.
	 컬렉션은 제네릭을 기반으로 하므로 인스턴스 생성시 저장할 타입을 결정한다.
	 여기서는 Friend클래스를 상속한 HighFriend, UnivFriend를 저장할 것이므로 
	 이와 같이 선언하면 된다.
	 */
...
//		numOfFriends =0;
	...
	public void showAllData() {
		/*일반for문을 사용하는 경우 변수i를 인덱스로 사용해야 하므로
	...
	public void showSimpleData() {
		/*
		 확장for문은 인덱스를 사용하지 않으므로 간단히 인스턴스를 인출가능
		 */
		for(Friend fr: lists) {
			fr.showBasicInfo();
		}
